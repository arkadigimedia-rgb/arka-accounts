...
export async function GET() {
  if (demoModeEnabled()) {
    return NextResponse.json(demoPayments().filter((p) => p.status === "DUE_TODAY"));
  }

  if (!process.env.DATABASE_URL) {
    return NextResponse.json(operationalStore.getPayments().filter((p) => p.status === "DUE_TODAY"));
  }
